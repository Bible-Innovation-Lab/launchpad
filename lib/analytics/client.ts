/**
 * @bil/analytics — client beacon.
 *
 * ~1 KB module. Single function: `track(event, props?)`. POSTs to /api/track.
 * Fire-and-forget; analytics failures never break the user-facing app.
 *
 * The server endpoint reads the `_lp_aid` cookie (set by middleware on first
 * page load) and enriches with geo + parsed UA before forwarding to PostHog.
 * The client never sees the anon-id and never sets the cookie.
 *
 * @example
