...
- `FeedbackModal`: portal to `document.body` with higher z-index so fixed app chrome (banners, transformed ancestors) no longer swallows submit clicks; submit uses a real `<form>` for Enter-to-send.
