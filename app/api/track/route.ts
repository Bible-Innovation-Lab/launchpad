 ...
 * cookie set by middleware, enriches the inbound event with geo + parsed
 * UA, forwards to PostHog. Emits `first_visit` first when middleware
 * signals via the `x-lp-first-visit` header on cookie mint.
 */

import { NextRequest, NextResponse } from "next/server";
import { capture, parseUA } from "@/lib/analytics/server";
import type { JSONValue } from "@/lib/analytics/client";
...
export async function POST(req: NextRequest) {
  ...
  if (!anonId) {
    // No cookie means middleware didn't mint one. Bot, prefetch, or curl.
  ...
  // If middleware just minted the cookie (one-shot _lp_fv=1), emit
  // first_visit BEFORE the inbound event so funnel ordering is preserved.
